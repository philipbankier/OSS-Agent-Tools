...
func Run(opts Options) (*Result, error) {
	...
	picoClawHome, err := resolveAutoClawHome(opts.AutoClawHome)
	if err != nil {
		return nil, err
	}
	...
	fmt.Println("Migrating from OpenClaw to AutoClaw")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Description: "convert OpenClaw config to AutoClaw format",
			})
...
func executeConfigMigration(srcConfigPath, dstConfigPath, picoClawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing AutoClaw config: %w", err)
		}
...
func resolveAutoClawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	...
	return filepath.Join(home, ".autoclaw"), nil
}
