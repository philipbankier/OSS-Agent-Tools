...
beforeEach(() => {
  ...
  // Create the three-space layout
  ensureDir(join(tempDir, '.tastekit', 'self'));
  ensureDir(join(tempDir, '.tastekit', 'knowledge', 'skills'));
  ensureDir(join(tempDir, '.tastekit', 'knowledge', 'playbooks'));
  ensureDir(join(tempDir, '.tastekit', 'ops'));
});
...
describe('compileSkills', () => {
  it('generates manifest for content-agent domain', async () => {
    ...
    const manifestPath = join(tempDir, '.tastekit', 'knowledge', 'skills', 'manifest.v1.yaml');
  ...
  it('writes SKILL.md files for each domain skill', async () => {
    ...
    for (const skill of manifest.skills) {
      const skillMdPath = join(tempDir, '.tastekit', 'knowledge', 'skills', skill.skill_id, 'SKILL.md');
  ...
  it('generic skill includes constitution principles', async () => {
    ...
    const skillMdPath = join(tempDir, '.tastekit', 'knowledge', 'skills', 'general-task', 'SKILL.md');
    const content = readFileSync(skillMdPath, 'utf-8');
