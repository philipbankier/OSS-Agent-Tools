...
describe('tastekit session path contracts', () => {
  it('uses ops/session.json as canonical onboarding session location', async () => {
    const root = makeTempWorkspace('session-contract-path');
    const tastekitDir = join(root, '.tastekit');
    const opsDir = join(tastekitDir, 'ops');
    const sessionPath = join(opsDir, 'session.json');
    ...
    try {
      ...
      expect(existsSync(join(root, '.tastekit', 'self', 'constitution.v1.json'))).toBe(true);
    } finally {
