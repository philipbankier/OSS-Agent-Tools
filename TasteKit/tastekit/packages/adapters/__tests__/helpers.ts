...
export function createProfileFixture(layout: Layout): { rootDir: string; profilePath: string } {
  ...
  if (layout === 'v2') {
    mkdirSync(join(profilePath, 'self'), { recursive: true });
    mkdirSync(join(profilePath, 'knowledge', 'skills', 'demo-skill'), { recursive: true });
    mkdirSync(join(profilePath, 'knowledge', 'playbooks'), { recursive: true });

    writeFileSync(join(profilePath, 'self', 'constitution.v1.json'), JSON.stringify(CONSTITUTION, null, 2), 'utf-8');
    writeFileSync(join(profilePath, 'self', 'guardrails.v1.yaml'), GUARDRAILS_YAML, 'utf-8');
    writeFileSync(join(profilePath, 'self', 'memory.v1.yaml'), MEMORY_YAML, 'utf-8');
    writeFileSync(join(profilePath, 'knowledge', 'skills', 'manifest.v1.yaml'), SKILLS_MANIFEST_YAML, 'utf-8');
    writeFileSync(join(profilePath, 'knowledge', 'skills', 'demo-skill', 'SKILL.md'), '# Demo Skill\n', 'utf-8');
    writeFileSync(join(profilePath, 'knowledge', 'playbooks', 'demo-playbook.v1.yaml'), PLAYBOOK_YAML, 'utf-8');
  } else {
    mkdirSync(join(profilePath, 'artifacts', 'playbooks'), { recursive: true });
    mkdirSync(join(profilePath, 'skills', 'demo-skill'), { recursive: true });

    writeFileSync(join(profilePath, 'artifacts', 'constitution.v1.json'), JSON.stringify(CONSTITUTION, null, 2), 'utf-8');
    writeFileSync(join(profilePath, 'artifacts', 'guardrails.v1.yaml'), GUARDRAILS_YAML, 'utf-8');
    writeFileSync(join(profilePath, 'artifacts', 'memory.v1.yaml'), MEMORY_YAML, 'utf-8');
    ...
    writeFileSync(join(profilePath, 'artifacts', 'playbooks', 'demo-playbook.v1.yaml'), PLAYBOOK_YAML, 'utf-8');
  }
